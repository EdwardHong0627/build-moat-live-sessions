...
(CON-001, Security). Strategy B uses OpenAI for BOTH embeddings and the final answer."""
import os
...
class Settings:
    ...
    # Cosine similarity is in [-1, 1]; 0.25 is a permissive starting floor for
    # cannot-confirm. Tune against the sample docs (FR-016, guards semantic false positives).
    score_threshold: float = field(default_factory=lambda: float(os.environ.get("SCORE_THRESHOLD", "0.25")))
    chunk_max_chars: int = field(default_factory=lambda: int(os.environ.get("CHUNK_MAX_CHARS", "1000")))
    ...
    def faiss_dir(self) -> str:
        return os.path.join(self.kb_dir, "faiss_index")
